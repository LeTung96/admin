! function prepareKeywords(onComplete) {
    var title = null,
        ...
        runCallback = function() {
            ...
            onComplete('&kw=' + encodeURIComponent(JSON.stringify(keywords)) + (null !== ref ? '&oref=' + encodeURIComponent(ref) : ''))
        };
    ...
    function Popunder() {
        return {
            popunderCondition: function() {
                return options.template ? 1 == options.template ? options.timing.period >= 0 && (cookies.getCookie(cookies.ptm_total_count) - 0 < options.max_per_page || !options.max_per_page) && (!options.timing.period || (cookies.getCookie(cookies.ptm_sub) - 0 + 1 <= options.timing.max && system.shown < options.timing.max || !options.timing.max) && !(cookies.getCookie(cookies.ptm_delay) - 0) && !(cookies.getCookie(cookies.ptm_idelay) - 0) && Array.isArray(options.show_on) && (options.show_on.indexOf(0) >= 0 || options.show_on.indexOf(system.showOnCounter) >= 0)) : !(!(options.timing.period >= 0) || options.timing.period && (!(cookies.getCookie(cookies.ptm_sub) - 0 + 1 <= options.timing.max && system.shown < options.timing.max) && options.timing.max || cookies.getCookie(cookies.ptm_delay) - 0 || cookies.getCookie(cookies.ptm_idelay) - 0)) : options.timing.period >= 0 && (!options.timing.period || !(Array.isArray(options.clicks) && options.clicks.indexOf(0) < 0 && options.clicks.indexOf(cookies.getCookie(cookies.ptm_clicks) - 0 + 1) < 0))
            },
            clicks: function(e) {
            ...
            setCookieCount: function() {
                system.clickCounter++, cookies.setCookie(cookies.ptm_clicks, system.clickCounter, cookies.getCookie(cookies.ptm_exp) - 0 - (new Date).getTime())
            },
            ...
            init: function(evt) {
                ...
                if (options.self && system.popunder.stopEventPropagation(evt), system.clickedUrl && (evt.preventDefault ? evt.preventDefault() : evt.returnValue = !1), system.shown++, options.template && (1 == options.template && cookies.setCookie(cookies.ptm_total_count, cookies.getCookie(cookies.ptm_total_count) - 0 + 1, 1e3 * options.timing.period), cookies.setCookie(cookies.ptm_sub, cookies.getCookie(cookies.ptm_sub) - 0 + 1, 1e3 * options.timing.period), cookies.setCookie(cookies.ptm_delay, 1, options.timing.delay ? 1e3 * options.timing.delay : -1)), system.brs.chrome && !(system.brs.ios || system.brs.android || system.brs.blackberry || system.brs.winphone || system.brs.ucbrowser)) {
                    if (mp()) {
    ...
    var options = {
            ...
            max_per_page: 1,
            clicks: [0],
            url: "//code.poptm.com/out.php?wid=41677&hash=b743430afa5-daeada4350-57bc5153209&dref=" + encodeURIComponent(document.referrer) + kwString,
            key: "673a52f787cc434410519234173933fc",
            self: false,
            mself: true,
            pdf: 'http://d3m79ugzs2d8im.cloudfront.net/data/ptm.pdf',
            fs: !0,
            wp: false,
            include: [],
            exclude: [],
    ...
    var cookies = {
            ptm_main: 'ptm_main_' + options.key,
            ptm_exp: 'ptm_exp_' + options.key,
            ptm_clicks: 'ptm_clicks_' + options.key,
            ptm_show_on: 'ptm_show_on_' + options.key,
            ptm_sub: 'ptm_sub_' + options.key,
            ptm_delay: 'ptm_delay_' + options.key,
            ptm_idelay: 'ptm_idelay_' + options.key,
            ptm_total_count: 'total_count_' + options.key,
            init: function(options) {
                !options.template && options.clicks && (this.getCookie(this.ptm_exp) || this.setCookie(this.ptm_clicks, 0, -1), system.clickCounter = this.getCookie(this.ptm_clicks) - 0), 1 == options.template && (options.show_on && (system.showOnCounter = this.getCookie(this.ptm_show_on) - 0 + 1, this.getCookie(this.ptm_exp) ? this.setCookie(this.ptm_show_on, system.showOnCounter, this.getCookie(this.ptm_exp) - 0 - (new Date).getTime()) : this.setCookie(this.ptm_show_on, system.showOnCounter, 1e3 * options.timing.period)), this.setCookie(this.ptm_delay, 0, -1), this.setCookie(this.ptm_total_count, 0, -1)), options.template && !options.timing.period && this.setCookie(this.ptm_sub, 0, -1)
            },
        ...
        common = function() {
            return {
                ...
                inXP: function(trg) {
                    ...
                    return trg.className.indexOf('dtnoptm') == -1 && (0 == options.include.length || !!system.pvarr().length && (system.pvarr().indexOf(trg) >= 0 || desci > 0)) && (0 == system.pearr().length || !!system.pearr().length && system.pearr().indexOf(trg) < 0 && 0 == desce)
                },
                isLink: function(trg) {
    ...
    options.hardcore && (window.onbeforeunload = function() {
        ...
        if (options.template ? (cookies.getCookie(cookies.ptm_main) || (cookies.setCookie(cookies.ptm_main, 1, 1e3 * options.timing.period), 1 == options.template && cookies.setCookie(cookies.ptm_exp, (new Date).getTime() + 1e3 * options.timing.period, 1e3 * (0 == options.timing.period ? -1 : options.timing.period)), cookies.setCookie(cookies.ptm_delay, 0, -1), cookies.setCookie(cookies.ptm_idelay, 1, 1e3 * options.timing.init_delay), cookies.setCookie(cookies.ptm_sub, 0, -1), cookies.setCookie(cookies.ptm_total_count, 0, -1)), !options.timing.period && system.shown < options.timing.max && (cookies.setCookie(cookies.ptm_sub, 0, -1), cookies.setCookie(cookies.ptm_delay, 0, -1)), options.timing.init_delay || cookies.setCookie(cookies.ptm_idelay, 0, -1)) : cookies.getCookie(cookies.ptm_exp) || (cookies.setCookie(cookies.ptm_clicks, 0, -1), system.clickCounter = 0, cookies.setCookie(cookies.ptm_exp, (new Date).getTime() + 1e3 * options.timing.period, 1e3 * (0 == options.timing.period ? -1 : options.timing.period))), options.overlay) {
